...
class QTFormatterImpl implements QTFormatter {
  ...
  QTFormatterImpl(this.centerX, this.centerY, this.smallestX, this.smallestY, this.formatX, this.formatY);

  /// Creates a coordinate converter.
  /// [smallest] is the precision of the coordinate components, smallest X and Y change.
  factory QTFormatterImpl.Origin(double smallest, [String format = _defaultFormat]) {
    final NumberFormat numFmt = NumberFormat(format, "en_US");
    return QTFormatterImpl(0.0, 0.0, smallest, smallest, numFmt, numFmt);
  }
  ...
  /// [centerY] is the second component (Y) of the center point of the coordinate system.
  /// [smallest] is the precision of the coordinate components, smallest X and Y change.
  factory QTFormatterImpl.Symmetric(double centerX, double centerY, double smallest, [String format = _defaultFormat]) {
    final NumberFormat numFmt = NumberFormat(format, "en_US");
  ...
  QTEdgeImpl toEdge(double x1, double y1, double x2, double y2) =>
      QTEdgeImpl(QTPointImpl(fromX(x1), fromY(y1)), QTPointImpl(fromX(x2), fromY(y2)));

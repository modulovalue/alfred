import 'package:three_dart/parser/calculator.dart' as calculator;
import 'package:three_dart/parser/diff.dart' as _diff;
import 'package:three_dart/parser/grammar.dart' as _grammar;
import 'package:three_dart/parser/parser.dart' as _parser;
...
class TestArgs {
  ...
  void log(String msg) => this._buf.writeln(msg);
  ...
  void checkRuleString(_grammar.Rule rule, int index, String exp) {
    final String result = rule.toString(index);
    if (exp != result) {
  ...
  void checkDiff(List<String> a, List<String> b, List<String> exp) {
    final String result = _diff.plusMinusParts(a, b).join('|');
    final String expStr = exp.join('|');
    if (expStr != result) {
  ...
  void checkGrammar(_grammar.Grammar grammar, List<String> expected) {
    final String exp = expected.join('\n');
    final String result = grammar.toString().trim();
    if (exp != result) {
      String diff = _diff.plusMinusLines(exp, result);
  ...
  void checkTermFirst(_grammar.Grammar grammar, String token, List<String> expected) {
    final String exp = expected.join('\n');
    final List<_grammar.TokenItem> firsts = grammar.term(token).determineFirsts();
    final String result = firsts.join('\n');
    if (exp != result) {
      String diff = _diff.plusMinusLines(exp, result);
      diff = diff.trimRight().replaceAll('\n', '\n         ');
  ...
  void checkTermFollow(_grammar.Grammar grammar, String token, List<String> expected) {
    final String exp = expected.join('\n');
    final List<_grammar.TokenItem> firsts = grammar.term(token).determineFollows();
    final String result = firsts.join('\n');
    if (exp != result) {
      String diff = _diff.plusMinusLines(exp, result);
      diff = diff.trimRight().replaceAll('\n', '\n         ');
  ...
  void checkTok(tokenizer.Tokenizer tok, String input, List<String> expected) {
    final StringBuffer resultBuf = StringBuffer();
    for (final tokenizer.Token token in tok.tokenize(input)) {
      resultBuf.writeln(token.toString());
    }
    final String exp = expected.join('\n');
    final String result = resultBuf.toString().trim();
    if (exp != result) {
      String diff = _diff.plusMinusLines(exp, result);
      diff = diff.trimRight().replaceAll('\n', '\n         ');
  ...
  void checkParser(_parser.Parser parser, List<String> input, List<String> expected) {
    final _parser.Result parseResult = parser.parse(input.join('\n'));
    final String exp = expected.join('\n');
    final String result = parseResult.toString();
    if (exp != result) {
      String diff = _diff.plusMinusLines(exp, result);
      diff = diff.trimRight().replaceAll('\n', '\n        ');
  ...
  void checkParserBuildError(_grammar.Grammar grammar, tokenizer.Tokenizer tokenizer, List<String> expected) {
    final String exp = expected.join('\n');
    try {
      _parser.Parser.fromGrammar(grammar, tokenizer);
      this.error('Expected an exception from parser builder but got none:' + '\n  Expected: $exp');
    } on Object catch (err) {
      final String result = '$err'.trimRight();
      if (exp != result) {
        String diff = _diff.plusMinusLines(exp, result);
        diff = diff.trimRight().replaceAll('\n', '\n        ');
  ...
  void checkCalc(calculator.Calculator calc, String input, List<String> expected) {
    ...
    final String result = calc.stackToString;
    final String exp = expected.join('\n');
    if (exp != result) {
      String diff = _diff.plusMinusLines(exp, result);
      diff = diff.trimRight().replaceAll('\n', '\n        ');
...
class TestTool {
  ...
  void printResult() => print(this._failed ? 'FAILED' : 'PASSED');

  /// Runs a test given the test function.
  void run(dynamic Function(TestArgs args) test) {
    final TestArgs args = TestArgs();
    test(args);
    if (args.failed) {
...
void tokenizer00(TestArgs args) {
  ...
  final tokenizer.Tokenizer tok = tokenizer.Tokenizer();
  tok.start("start");
  ...
  args.checkTok(tok, "a + b * c", ['[id]:1:"a"', '[add]:3:"+"', '[id]:5:"b"', '[mul]:7:"*"', '[id]:9:"c"']);
  args.checkTok(tok, "(a + b)", ['[open]:1:"("', '[id]:2:"a"', '[add]:4:"+"', '[id]:6:"b"', '[close]:7:")"']);
...
void parser00(TestArgs args) {
  ...
  // 4. T → + T
  // 5. T → T + n
  final _grammar.Grammar grammar = _grammar.Grammar();
  grammar.start("E");
  ...
  final _parser.Parser parser = _parser.Parser.fromGrammar(grammar, tok);
  args.checkParser(parser, ["103"], ['─<E>', '  └─<T>', '     └─[n:3:"103"]']);
  args.checkParser(parser, ["+2"], ['─<E>', '  └─<T>', '     ├─[+:1:"+"]', '     └─<T>', '        └─[n:2:"2"]']);
...
void parser04(TestArgs args) {
  ...
  _parser.Parser parser = _parser.Parser.fromGrammar(grammar, tok);
  // Test serializing and deserializing too.
  final String data = parser.serialize().toString();
  parser = _parser.Parser.deserialize(simple.Deserializer(data));
...
void loader00(TestArgs args) {
  args.log('loader00');
  final _parser.Parser parser = _parser.Loader.getParser();
  args.checkParser(parser, [
    "()"
  ], [
...
void loader08(TestArgs args) {
  args.log('loader08');
  final _parser.Parser parser = _parser.Parser.fromDefinition([
...
void grammar00(TestArgs args) {
  args.log('grammar00');
  final _grammar.Grammar gram = _grammar.Grammar();
  gram.start("defSet");
...
void grammar01(TestArgs args) {
  args.log('grammar01');
  final _grammar.Grammar gram = _grammar.Grammar();
  final _grammar.Rule rule0 = gram.newRule('E');
  final _grammar.Rule rule1 = gram.newRule('E').addTerm("E").addToken("+").addTerm("E");
  final _grammar.Rule rule2 = gram.newRule('E').addTerm("E").addToken("+").addTerm("E").addTrigger('add');
  final _grammar.Rule rule3 = gram.newRule('E').addTerm("E").addToken("+").addTrigger('add').addTerm("E");
  final _grammar.Rule rule4 = gram.newRule('E').addTrigger('add').addTerm("E").addToken("+").addTerm("E");
...
void calc00(TestArgs args) {
  args.log('calc00');
  final calculator.Calculator calc = calculator.Calculator();
...
void calc03(TestArgs args) {
  args.log('calc03');
  final calculator.Calculator calc = calculator.Calculator();
  args.checkCalc(calc, 'square(11)', ['Errors in calculator input:', 'Exception: No function called square found.']);
  calc.addFunc("square", (List<Object?> list) {
    if (list.length != 1) throw Exception('Square may one and only one input.');
    final calculator.Variant v = calculator.Variant(list[0]);
  ...
  args.checkCalc(calc, 'square("cat")',
      ['Errors in calculator input:', 'Exception: May only square an int or real number but got String(cat).']);
}
...
void calc04(TestArgs args) {
  ...
  args.checkCalc(calc, 'sub("catch", 3, 1)', ['Errors in calculator input:', 'RangeError: Value not in range: 3']);
...
void calc08(TestArgs args) {
  ...
  args.checkCalc(calc, 'trim(str(1))', ['Errors in calculator input:', 'Exception: No function called str found.']);
}

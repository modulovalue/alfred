...
class SvgRenderer extends PlotterRenderer {
  /// The buffer to temporarily store SVG while drawing.
  final StringBuffer _sout;
  ...
  final void Function(String) render;
  ...
  void reset(
    ...
    _window = window;
    transform = trans;
    renderReset(window, _getColorString(backgroundColor));
    _sout.clear();
  }
  ...
  set fillColor(
    ...
    _fillClr = color;
    if (color != null) {
      final fillClr = _getColorString(color);
  ...
  void drawText(
    ...
    _sout.write("<text x=\"$x\" y=\"$y\" style=\"font-family: ${font}; font-size: ${size}px;\" ");
  ...
  void drawStrip(
    ...
    if (count >= 2) {
      double x = _transX(xCoords[0]);
      double y = _transY(yCoords[0]);
      _sout.write("<polyline points=\"" + x.toString() + "," + y.toString());
      for (int i = 1; i < count; ++i) {
        x = _transX(xCoords[i]);
        y = _transY(yCoords[i]);
        _sout.write(" $x,$y");
  ...
  void finalize() => render(_sout.toString());
